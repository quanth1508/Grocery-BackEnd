...
async function getStatisticRevenue(req, res) {
    try {
        if (!req.query) { throw THQError("Không có dữ liệu") }
        
    const FIRST_MONTH = 1
    const LAST_MONTH = 12
    var _month = []
    for (let i = 1; i <= 12; i++) { _month.push(`Tháng ${i}`) }
    const MONTHS_ARRAY = _month
    let TODAY = req.query.yearString
    let YEAR_BEFORE = req.query.yearBeforeString
    ...
    res.status(200).send(new Response(
        true,
        "Lấy thông số doanh thu tháng thành công",
        result
    ))
...
async function getCapitalAndRevenue(req, res) {
    try {
        ...
        const capital = await transactionModel.aggregate( [{ 
            $match: { }
          },
          { 
              $group: {
    ...
    res.status(200).send(new Response(
        true,
        "Lấy danh thông số xu hướng lợi nhuận thành công",
        {
            revenue,
            capital,
        }
